    ...
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            ...
            for user_id, user_data in users.items():
                ...
                if "last_daily_bonus" not in user_data:
                    user_data["last_daily_bonus"] = 0.0
                if "subscribed" not in user_data:
                    user_data["subscribed"] = False
                if "invited_by" not in user_data:
                    user_data["invited_by"] = None
                if "hide_in_leaderboard" not in user_data:
                    user_data["hide_in_leaderboard"] = False
    else:
        users = {}
        jackpot = 5000
        casino_balance = 0
...
async def handle_game(message: Message):
    ...
    if text in games:
        current_game[user_id] = games[text]
        await message.reply(
    ...
    if text in predefined_bets:
        ...
        await message.reply(
            f"{hbold('Ошибка:')} Выбери игру или используй кнопки для ставки!",
            parse_mode="HTML"
        )
        return
    ...
    print(f"Game: {current_game[user_id]}, Dice value: {dice_value}")
    ...
    if current_game[user_id] == "slot":
        ...
        if current_game[user_id] == "dice":
            winnings, result_message = determine_dice_win(dice_value, bet)
        elif current_game[user_id] == "basketball":
            winnings, result_message = determine_basketball_win(dice_value, bet)
        elif current_game[user_id] == "bowling":
            winnings, result_message = determine_bowling_win(dice_value, bet)
        elif current_game[user_id] == "football":
            winnings, result_message = determine_football_win(dice_value, bet)
        elif current_game[user_id] == "darts":
            winnings, result_message = determine_darts_win(dice_value, bet)
